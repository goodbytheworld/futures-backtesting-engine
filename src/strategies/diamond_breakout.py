"""
Diamond-top / diamond-bottom breakout strategy.
"""
...
@dataclass
class DiamondBreakoutConfig:
    """
    Parameters for the diamond breakout strategy.

    Methodology:
        The strategy looks for a completed expanding-then-contracting diamond
        structure and trades the breakout of its final contracting boundary
        with volume confirmation.
    """

    pattern_window: int = 24
    pattern_memory: int = 6
    trend_window: int = 20
    min_slope_atr_ratio: float = 0.15
    max_contraction_ratio: float = 0.75
    min_height_atr: float = 2.50
    ...
    shock_max_close_change_atr: float = 2.50


class DiamondBreakoutStrategy(BaseStrategy):
    """
    Trades breakouts from practical diamond-top and diamond-bottom structures.
    """
    ...
    def __init__(
        self,
        engine,
        config: Optional[DiamondBreakoutConfig] = None,
    ) -> None:
        super().__init__(engine)
        cfg = config or DiamondBreakoutConfig()
        apply_wfo_dataclass_overrides(engine, cfg, "dmnd")
        self.config = cfg
        ...
        diamond = detect_diamond_patterns(
            high=high,
            low=low,
            close=close,
            atr=atr,
            window=cfg.pattern_window,
            trend_window=cfg.trend_window,
            min_slope_atr_ratio=cfg.min_slope_atr_ratio,
            max_contraction_ratio=cfg.max_contraction_ratio,
            min_height_atr=cfg.min_height_atr,
        )

        recent_bottom = (
            diamond.bottom_pattern.shift(1)
            .rolling(cfg.pattern_memory, min_periods=1)
            .max()
            .fillna(0.0)
            > 0.0
        )
        recent_top = (
            diamond.top_pattern.shift(1)
            .rolling(cfg.pattern_memory, min_periods=1)
            .max()
            .fillna(0.0)
            > 0.0
        )
        upper_boundary = diamond.upper_boundary.shift(1)
        lower_boundary = diamond.lower_boundary.shift(1)
        pattern_height = diamond.height.shift(1)
        body_atr = (close - open_).abs() / atr.replace(0.0, float("nan"))

        long_sig = (
            recent_bottom
            & upper_boundary.notna()
            & (close > upper_boundary)
            & (volume_ratio >= cfg.breakout_volume_ratio)
            & (body_atr >= cfg.breakout_body_atr)
            & (close > open_)
        )
        short_sig = (
            recent_top
            & lower_boundary.notna()
            & (close < lower_boundary)
            & (volume_ratio >= cfg.breakout_volume_ratio)
            & (body_atr >= cfg.breakout_body_atr)
            & (close < open_)
        )

        projected_move = pd.concat([pattern_height, atr], axis=1).max(axis=1)

        self._long_sig = long_sig.fillna(False)
        self._short_sig = short_sig.fillna(False)
        self._long_entry_stop = upper_boundary + entry_buffer
        self._short_entry_stop = lower_boundary - entry_buffer
        self._long_stop_loss = lower_boundary - atr * float(cfg.stop_atr_buffer)
        self._short_stop_loss = upper_boundary + atr * float(cfg.stop_atr_buffer)
        ...
        print(
            f"[Diamond Breakout] Ready | window={cfg.pattern_window} | "
            f"long={int(self._long_sig.sum()):,} short={int(self._short_sig.sum()):,}"
        )
    ...
    def get_search_space(cls) -> Dict[str, Any]:
        return {
            "dmnd_pattern_window": (16, 36, 4),
            "dmnd_pattern_memory": (3, 8, 1),
            "dmnd_breakout_volume_ratio": (1.10, 1.80, 0.10),
            "dmnd_min_height_atr": (2.00, 4.00, 0.25),
            "dmnd_target_rr": (1.25, 2.25, 0.25),
        }
